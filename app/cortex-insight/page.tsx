...
export const metadata: Metadata = {
  title: "Cortex Insight: Conversational Market Research — CortexCraft.AI",
  description:
    "Cortex Insight automates market research with AI. Conduct interviews, synthesize insights, and deploy inside your stack in days.",
};
...
async function getPageContent(): Promise<PageContent | null> {
  try {
    const res = await fetch("http://localhost:8000/api/cms/product/cortex-insight/", {
      next: { revalidate: 0 },
    });
    if (!res.ok) throw new Error(`API returned ${res.status}`);
    return await res.json();
  } catch (err) {
    console.warn("[CMS] Cortex Insight API unavailable:", err);
    return null;
  }
}
...
const FALLBACK: PageContent = fallbackData.cortexInsight;

// ── Page ──────────────────────────────────────────────────────────────────

export default async function CortexInsightPage() {
